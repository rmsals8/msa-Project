...
public class RouteMetricsAnalyzer {
    ...
    private TransportMode determineTransportMode(double distance, LocalDateTime time) {
        ...
        if (distance <= 3.0)
            return isRushHour(time) ? TransportMode.SUBWAY : TransportMode.BUS;
    ...
    private String assessDifficulty(double distance, int duration, List<OptimizedSchedule> schedules) {
        if (distance <= EASY_DISTANCE_THRESHOLD && duration <= EASY_DURATION_THRESHOLD)
            return "EASY";
        if (distance <= MODERATE_DISTANCE_THRESHOLD && duration <= MODERATE_DURATION_THRESHOLD)
            return "MODERATE";
        return "HARD";
    }
    ...
    private String determineCategory(String name) {
        if (name.contains("궁") || name.contains("박물관"))
            return "문화/역사";
        if (name.contains("시장") || name.contains("상가"))
            return "쇼핑";
        if (name.contains("공원") || name.contains("광장"))
            return "휴식";
        return "기타";
    }
