...
import com.example.auth_service.schedule_service.dto.response.RouteAnalysisResponse;
import com.example.auth_service.schedule_service.dto.response.route.RoutePlaceDetailResponse;
import com.example.auth_service.schedule_service.dto.response.route.RouteRecommendationResponse;
import com.example.auth_service.schedule_service.dto.response.route.RouteSegmentDetailResponse;
import com.example.auth_service.schedule_service.exception.RouteAnalysisException;
...
public class RouteRecommendationService {
    ...
    public RouteRecommendationResponse generateRouteRecommendation(RouteRecommendationRequest request) {
        ...
        try {
            // 비동기로 각 분석 수행
            CompletableFuture<List<RouteSegmentDetailResponse>> segmentsFuture = CompletableFuture
                    .supplyAsync(() -> analyzeRouteSegments(request.getOptimizedSchedules()));

            CompletableFuture<List<RoutePlaceDetailResponse>> placesFuture = CompletableFuture
                    .supplyAsync(() -> analyzePlaceDetails(request.getOptimizedSchedules()));

            CompletableFuture<RouteAnalysisResponse> analysisFuture = CompletableFuture
                    .supplyAsync(() -> generateRouteAnalysis(request.getOptimizedSchedules()));
