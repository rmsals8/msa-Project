...
import com.example.auth_service.navigation_service.dto.request.LocationUpdate;
import com.example.auth_service.navigation_service.dto.request.StartNavigationRequest;
import com.example.auth_service.navigation_service.dto.response.NavigationStatus;
import com.example.auth_service.navigation_service.dto.response.navigation.RouteDetail;
import com.example.auth_service.navigation_service.dto.response.navigation.RouteOptionSummary;
import com.example.auth_service.navigation_service.dto.response.navigation.RouteOptionsResponse;
import com.example.auth_service.navigation_service.dto.response.navigation.RouteSegment;
import com.example.auth_service.navigation_service.dto.response.navigation.RouteStep;
import com.example.auth_service.navigation_service.dto.response.navigation.TotalOptions;
import com.example.auth_service.navigation_service.dto.response.navigation.TransportOption;
import com.example.auth_service.navigation_service.service.NavigationService;
import com.example.auth_service.navigation_service.service.TmapService;
import com.example.auth_service.common.dto.domain.route.TransportMode;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;

import com.example.auth_service.common.exception.NavigationException;
...
public class NavigationController {
                                                ...
                                                RouteDetail.builder()
                                                        ...
                                                        .crowdedness(
                                                                drivingRoute.containsKey("trafficInfo")
                                                                        ? ((Map<String, Object>) drivingRoute
                                                                                .get("trafficInfo"))
                                                                                .get("status").toString()
                                                                        : "MODERATE")
                                                        .build()))
    ...
    private RouteOptionSummary findFastestOption(List<RouteSegment> segments) {
        ...
        RouteSegment fastestSegment = segments.stream()
                .min(Comparator.comparingInt(segment -> segment.getOptions().get(0).getRoutes().get(0).getDuration()))
    ...
    private RouteOptionSummary findCheapestOption(List<RouteSegment> segments) {
        ...
        RouteSegment cheapestSegment = segments.stream()
                .min(Comparator.comparingDouble(segment -> segment.getOptions().get(0).getRoutes().get(0).getCost()))
                .orElseThrow(() -> new NavigationException("경로를 찾을 수 없습니다"));
