...
public interface SavedScheduleRepository extends JpaRepository<SavedSchedule, Long> {
    ...
    // JPQL을 사용한 새로운 메서드 - items만 즉시 로딩하고 segments는 지연 로딩
    @Query("SELECT s FROM SavedSchedule s LEFT JOIN FETCH s.scheduleItems WHERE s.id = :id AND s.userNo = :userNo")
    Optional<SavedSchedule> findByIdAndUserNoWithItems(@Param("id") Long id, @Param("userNo") Long userNo);
